...
async function onRequest(context2) {
  ...
  console.log("Has Supabase URL?", !!env2.VITE_SUPABASE_URL);
  console.log("Has Supabase Key?", !!env2.VITE_SUPABASE_ANON_KEY);
  if (subdomain && subdomain !== "www" && env2.VITE_SUPABASE_URL && env2.VITE_SUPABASE_ANON_KEY) {
    try {
      const supabaseUrl = env2.VITE_SUPABASE_URL;
      const supabaseKey = env2.VITE_SUPABASE_ANON_KEY;
      ...
      if (supaRes.ok) {
        ...
        if (data && data.length > 0) {
          ...
          const title2 = `${groom} & ${bride} - Undangan Pernikahan`;
          ...
          console.log("Replacing tags for:", title2);
          return new HTMLRewriter().on("title", {
            element(element) {
              element.setInnerContent(title2);
            }
          }).on('meta[name="title"]', {
            element(element) {
              element.setAttribute("content", title2);
            }
...
// ../.wrangler/tmp/pages-41Zcfz/functionsRoutes-0.6986721477605513.mjs
var routes = [
  {
    routePath: "/:path*",
    mountPath: "/",
