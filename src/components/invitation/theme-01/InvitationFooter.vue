...
<template>
  <footer 
    ref="targetContainer"
    :class="[
        'py-10 text-center bg-wedding-dark text-white border-t border-zinc-800 transition-all duration-800 ease-out will-change-[opacity,transform]',
        isIntersecting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'
    ]"
  >
      <p class="text-xs text-zinc-400 mb-2 whitespace-pre-line px-6">{{ footerClosing }}</p>
      <p class="text-xs text-zinc-400 mb-6">{{ footerThanks }}</p>
      <h2 class="text-2xl font-wedding-serif italic mb-6">{{ title }}</h2>
      
      <div class="mt-8 pt-8 border-t border-zinc-800 text-[10px] text-zinc-500 flex flex-col items-center gap-2">
